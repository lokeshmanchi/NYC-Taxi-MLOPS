...
You can run the NYC Taxi MLOps pipeline either locally or using Docker Compose. Below are the recommended steps for each approach:
...
1. **Build all services:**
    ...
    make build
    ```
2. **Start all services (MLflow, Prefect, API, Streamlit):**
    ```bash
    make up
    ```
3. **Run the training pipeline (in Docker):**
    ```bash
    make train
    ```
4. **View logs:**
...
5. **Stop all services:**
    ...
    make down
    ```
6. **Full reset (remove all volumes):**
    ```bash
    make clean
    ```

### **Notes**
- After retraining, restart the API service to load the new model:
  ```bash
  docker compose restart api
  ```
- The EDA dashboard expects pre-aggregated summary files at `data/summary/hourly_summary.parquet`.
- For distributed training (PyTorch DDP), see `src/training/train_pytorch_ddp.py` and adapt the launch command for your cluster environment.
