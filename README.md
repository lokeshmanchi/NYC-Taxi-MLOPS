...
- **MLflow (Model Registry + Experiment Tracking):** Single MLflow service serving two roles. As a model registry: stores and versions the `nyc-taxi-regressor` (sklearn pipeline + ONNX) and `nyc-taxi-regressor-ddp` (PyTorch + ONNX) artifacts. As an experiment tracker: receives step-level and epoch-level metrics from both training paths, and serves run data (metrics, params, feature importances) to the Model Performance dashboard.
...
- **GET /monitoring/drift:** Generates an [Evidently](https://docs.evidentlyai.com/) HTML data drift report. Uses `data/processed/` (Processed Data) as the **reference distribution** (sampled up to 5,000 rows) and `data/predictions/` (Prediction Logs) as the **current distribution**. Requires at least one `/predict` call before the endpoint can run.
- **EDA Dashboard (1_EDA.py):** Loads `data/summary/hourly_summary.parquet` (auto-generated by ETL) for exploratory data analysis.
- **Model Performance (2_Model_Performance.py):** Fetches run history and metrics from MLflow (dotted "Fetch run data" arrow), then loads the sklearn pipeline artifact (solid "Load pipeline" arrow) to extract the `XGBRegressor` step and display feature importances, MAE, RMSE, and R² across all training runs.
