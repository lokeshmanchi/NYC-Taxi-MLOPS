...
def train_task(data_path: str, model_output_path: str, mlflow_tracking_uri: str) -> str:
    from src.training.train import train

    logger = get_run_logger()
    logger.info(f"Starting training — data: {data_path}, model: {model_output_path}")
    run_id = train(
        data_path=data_path,
...
def training_pipeline(
    ...
    model_output_path = model_output_path or os.getenv("MODEL_OUTPUT_PATH", "models/model.pkl")
    mlflow_tracking_uri = mlflow_tracking_uri or os.getenv("MLFLOW_TRACKING_URI", "http://localhost:5000")
