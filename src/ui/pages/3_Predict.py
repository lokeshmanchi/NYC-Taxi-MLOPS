...
with st.form("prediction_form"):
    ...
    with col2:
        ...
        pickup_month = st.selectbox(
            "Month",
            [1, 2, 3, 4, 5],
            format_func=lambda x: ["Jan", "Feb", "Mar", "Apr", "May"][x - 1],
        )
    ...
    with st.spinner("Predicting…"):
        try:
            ...
            if response.status_code == 200:
                ...
                st.markdown("---")
                html = (
                    "<div style='text-align:center; padding:2rem; background:#0e1117; "
                    "border-radius:12px; border:1px solid #00CC96'>"
                    "<h2 style='color:#00CC96; margin:0'>Predicted Fare</h2>"
                    f"<h1 style='color:#ffffff; font-size:3.5rem; margin:0.5rem 0'>"
                    f"${result['predicted_fare']:.2f}</h1>"
                    "<p style='color:#888; margin:0'>USD · Base fare only (excluding tip, "
                    "surcharges, tolls)</p>"
                    "</div>"
                )
